...
- **账号**：`账号` 页面为每个平台粘贴 Cookie，或点 `扫码登录`
  ...
  与 ECHO 本体共用（加密存储，见下）。
- **歌曲详情页 MV 背景**：**默认关闭（可选开启）**。打开后**点进歌曲详情页（歌词页）就会用「歌曲名 + 艺人 + MV」在哔哩哔哩搜索，
  取第一个视频**，显示在歌词后面，并**跟随歌曲播放进度**。播放栏右侧的 **MV 按钮**就是这颗开关：
  打开时**亮色**（强调色 + 播放栏下方的小横线）、关闭时**暗色**，在主界面点它除了打开开关还会
  **自动进入歌曲详情页**（歌词页是唯一显示 MV 的页面；已经在详情页时只切换开关）；全部背景设置在自己的侧栏页 **「MV 背景」** 里（见下）。
...
- **我的歌单（账号歌单 / B 站收藏夹）**：侧栏「多平台音源」**打开就是这一页**（模组页默认视图 `state.view = 'playlists'`，
  同一次会话内记住你最后看的标签；页面根节点由 Loader 在每次路由切换时重新挂载，所以记住的是模块内的 `state.view`，
  不写盘）。登录后直接列出该账号自己的内容：网易云 / QQ 音乐的云歌单，以及 **B 站的收藏夹**、
  ...
  **点开歌单只读取，不会自动保存**；从歌单明细点 `← 返回` 会回到「我的歌单」页（不是搜索页）。
  - **这页不出现任何账号元素**：顶部的平台选择器（`renderProviderPicker({ compact: true })`）只显示平台名与当前高亮，
    不再显示「已登录 · 昵称」和状态点（完整芯片条只在 **搜索** 页）；未登录时整页只有一行 `mms-login-hint`
    —— 一句说明 + `去登录`（点了把 `state.view` 切到 `accounts`）。平台自己的「请先登录」错误文本也被这句替代。
...
- **每日推荐**：并进「我的歌单」当分组（不再是一个标签）：分组头有 `打开每日推荐` 与 `刷新`，
  正文预览前 5 首（`DAILY_PREVIEW_LIMIT`，直接用 `renderTrackRow`，行内 `▶` `+` 可用），
  余下的提示「还有 N 首」。数据来自 `refreshNeteaseDailyRecommend`，一次会话只读一次
  （`state.daily.loaded`），`打开每日推荐` 会 `force` 刷新并把结果作为歌单明细打开，可以整列表播放或加入队列。
  「我喜欢」不再需要单独的同步按钮 —— 它本来就是网易云账号歌单列表里的一项，在「我的歌单」里点开即可。

## 「MV 背景」页面

侧栏 **🎬 MV 背景**（`external.sidebar.register({ id: 'multi-music-sources-mv', … })`，与音频页并列的第二条），
里面能直接看到并调整全部背景行为：

- **总开关**：第一行就是 `启用 MV 背景`（同一个 `mvEnabled`，走 `setBackdropEnabled()` 而不是
  `persistBackgroundSettings()`，因为只有前者会真的启停背景层）。**背景关着时这一页照样能打开** ——
  歌词页的抽屉在背景关闭时是不存在的，所以设置入口不能只放在抽屉里。
...
**这一页怎么保持最新**：`renderBackgroundSettings({ masterSwitch: true })` 由 `renderBackgroundPage()`
渲染进自己的根节点（`backgroundPageRoot`，与 `pageRoot` 分开）。轮询定时器每 2 秒调一次
`refreshBackgroundPage()`，它按一个**内容签名**（引擎状态 / 当前选中 MV / 偏移 / 候选数量 / busy）决定要不要重建 ——
用户正在拖的滑条值不在签名里，所以重建永远不会跟拖动打架；`loadMvEngine()` 结束时再 `force` 刷一次，
所以 `刷新状态` 立刻可见。背景关闭时 `pollBackdrop()` 会提前返回，因此这个刷新是**放在它前面**单独调的。

### 歌词页右下角：快捷面板 + 完整设置抽屉

歌词页（歌曲详情页）右下角有一个 **MV 面板**：**整条标题都可以点击**展开/收起，标题右侧的 `⚙ 设置`
直接打开**右侧完整设置抽屉**——抽屉里就是「MV 背景」页的那一整套界面（画面预设、尺寸、填充、X-Y-Z、
...
| 设置 | 键 | 默认 | 范围 |
| --- | --- | --- | --- |
| 启用 MV 背景（= 播放栏 MV 按钮） | `mvEnabled` | 关（模组默认，社区版为开） | — |
...
> 背景只注入歌词页；其他页面不会出现视频。播放栏 MV 按钮的状态就是 `mvEnabled`（也是「MV 背景」页与配置弹窗里的那颗开关），
> 会持久化到模组配置；按钮的亮 / 暗只表示这颗开关，不表示此刻视频有没有在播。在主界面（非歌词页）点它会先跳到歌词页。
...
| 验证 | 覆盖内容 |
| --- | --- |
| `verify-renderer` | 在 DOM 桩里跑 `mod.js`：**两个侧栏条目**（`multi-music-sources` / `multi-music-sources-mv`）与它们的独立根节点、清理函数、搜索流程、错误提示、播放/入队调用、右键菜单。**RPC 响应按 Loader 真实的双层信封建模**；**默认落在「我的歌单」**、标签恰好是 `我的歌单 / 搜索 / 账号`（`背景设置`、`每日推荐` 不再是标签）、**会话内记住最后标签**（重新挂载页面后仍是账号页）、歌单页**只有一行登录提示且没有平台状态文字**、`去登录` 切到账号页、**每日推荐分组**（分组头 / 5 首预览 / `打开每日推荐` 打开明细）、搜索工具条不再有背景开关且刷新是图标按钮、**窄窗口音源下拉的媒体查询**、**MV 背景页带总开关**、背景层插在 ECHO 自己的 backdrop 之后（堆叠顺序回归）、社区版沉浸式 CSS 变量、播放栏开关用的是不会在窄窗口被隐藏的 `transport-media-button`、**我的歌单页（账号歌单 + B 站收藏夹卡片、点开导入并列出曲目、网易云条目走 `webUrl` 而不是裸 id、本地保存后再次打开不联网、卡面标「本地已存」、同步「我喜欢」按钮已移除）**、账号信息（头像 / 昵称 / UID / VIP）、扫码面板、**社区版获取流程（快照搜索 → 播放 `echo-mv://` 流）、引擎无流时回退到完整 MP4、分段流自动升级到完整 MP4、跟随 `positionSeconds` 与漂移方向、候选列表、自定义链接绑定、切换后 seek 一次到歌曲进度、画面预设写回并作用到 CSS、歌词页 ⚙ 打开完整设置抽屉、从 ECHO 自带界面播放的曲目会被登记且面板按钮可用、层被重建后会自动重新匹配、首次匹配失败后会自动重试而无需手动开关 MV**、**播放栏 MV 按钮按设置亮 / 暗（`data-active` + `is-soft-active` + `aria-pressed`，与「视频是否在播」无关）、切到「开」且不在歌词页时派发 `app:navigate:lyrics`（已经在详情页则只切开关、不派发）、出厂 `config.json` 的 `mvEnabled` 为 `false`** |
...
mod/                        模组包（打包进 .echomod 的内容）
  ...
  mod.js                    渲染进程页面（侧边栏「多平台音源」+「MV 背景」两条）
                            + 歌词页背景层 + 播放栏开关 + 状态提示条
